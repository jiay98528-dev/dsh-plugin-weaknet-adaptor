...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const inject = ['slots', 'remote', 'remote.weaknet', 'locale', 'timer'];
		...
		function apply(ctx) {
			...
			const remote = ctx.remote.weaknet;
			...
			function SettingsSection() {
				...
				React.useEffect(() => {
					...
					const loadAll = async () => {
						if (!alive) return;
						try {
							const [d, p] = await Promise.all([remote.getParamDefs(), remote.getParams()]);
							if (!alive) return;
					...
					const loop = async () => {
						if (!alive) return;
						try {
							const s = await remote.getStatus();
				...
				const save = async () => {
					...
					try {
						const r = await remote.setParams({ patch });
						if (r && r.ok) {
				...
				const resetDefaults = async () => {
					try {
						const r = await remote.setParams({ reset: true });
						if (r && r.ok) {
							const p = await remote.getParams();
							if (p && p.params) setForm(p.params);
							setNotice({ kind: 'ok', text: t('section.saved') });
						}
					} catch (e) {
			...
			function StatusDock() {
				...
				React.useEffect(() => {
					...
					const loop = async () => {
						if (!alive) return;
						try {
							const [s, p] = await Promise.all([remote.getStatus(), remote.getParams()]);
							if (!alive) return;
							setStatus(s);
							if (p && p.params) {
