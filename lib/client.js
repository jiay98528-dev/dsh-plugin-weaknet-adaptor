...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 注意：不能 inject 'remote.weaknet' —— 该命名空间由本插件自己的 $mount 创建，
		// inject 它会在 apply 前永远等待（死锁）。apply 内 $mount 后经 ctx.get 访问。
		const inject = ['slots', 'remote', 'locale', 'timer'];

		async function apply(ctx) {
			const locale = ctx.locale;

			// 注册 Remote contribution：安装 `remote.weaknet` 命名空间服务。
			// fail-soft：注册失败只降级设置页，绝不让 WebUI 卡死。
			try {
				await ctx.remote.$mount(WEAKNET_REMOTE);
			} catch (e) {
				console.error('weaknet: remote $mount failed', e);
			}
			...
			// $mount 完成后 `remote.weaknet` 作为 cordis 服务可用（key 'remote.weaknet'）
			const getRemote = () => ctx.get('remote.weaknet');
			...
			function SettingsSection() {
				...
				React.useEffect(() => {
					...
					const loadAll = async () => {
						if (!alive) return;
						try {
							const [d, p] = await Promise.all([getRemote()?.getParamDefs(), getRemote()?.getParams()]);
							if (!alive) return;
					...
					const loop = async () => {
						if (!alive) return;
						try {
							const s = await getRemote()?.getStatus();
				...
				const save = async () => {
					...
					try {
						const r = await getRemote()?.setParams({ patch });
						if (r && r.ok) {
				...
				const resetDefaults = async () => {
					try {
						const r = await getRemote()?.setParams({ reset: true });
						if (r && r.ok) {
							const p = await getRemote()?.getParams();
							if (p && p.params) setForm(p.params);
							setNotice({ kind: 'ok', text: t('section.saved') });
						}
					} catch (e) {
				...
				return React.createElement('div', { style: { padding: '8px 4px' } },
					React.createElement('h3', { style: { margin: '0 0 12px' } }, t('section.status')),
					...
					React.createElement('h3', { style: { margin: '0 0 12px' } }, t('section.params')),
					groups.map((g) => (defsByGroup[g] && defsByGroup[g].length > 0
			...
			function StatusDock() {
				...
				React.useEffect(() => {
					...
					const loop = async () => {
						if (!alive) return;
						try {
							const [s, p] = await Promise.all([getRemote()?.getStatus(), getRemote()?.getParams()]);
							if (!alive) return;
							setStatus(s);
							if (p && p.params) {
				...
				if (!enabled) return React.createElement('span', { style: { ...style, color: '#868e96' } }, t('dock.off'));
				...
				return React.createElement('span', { style: { ...style, color: STATUS_COLORS[sev], opacity: 0.9 } }, '● ' + text);
			}
