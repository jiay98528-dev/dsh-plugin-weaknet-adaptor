...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function apply(ctx) {
			...
			// 直接 RPC 调用桥：自定义通道 /weaknet（host 端 connection.rpc.handle 注册，
			// 不依赖 typert gateway 的 claims 可见性）。
			const weaknetCall = async (method, args) => {
				const result = await ctx.connection.rpc.call('/weaknet', method, { args }, undefined);
				if (!result.ok) {
